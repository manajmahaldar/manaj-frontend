        ...
        <div className="space-y-6">
            <div className="space-y-1">
                <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                    ...
                    {t.lh_pdfLibraryTitle || 'PDF Library'}
                </h1>
                <p className="text-xs font-semibold text-gray-500">
                    {t.lh_pdfLibraryDesc || 'Download and read offline guides, standard manuals, and feed spreadsheets.'}
                </p>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white p-12 rounded-3xl border border-gray-100 text-center">
                    <p className="text-gray-400 font-bold text-sm">{t.lh_noPdfs || 'No PDFs available matching your query.'}</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {pdfs.map(pdf => {
                        const locPdf = getLocalizedContent(pdf, language) || pdf;
                        return (
                            <div key={pdf._id} className="card bg-white p-6 rounded-3xl border border-gray-100 flex flex-col justify-between hover:shadow-lg transition-all duration-300">
                                <div>
                                    ...
                                    <h3 className="font-extrabold text-gray-900 text-base mb-2 line-clamp-2">{locPdf.title}</h3>
                                    <p className="text-xs text-gray-500 line-clamp-2 mb-4">
                                        {locPdf.description || 'Download this resource compiled by MatsyaLink Fisheries engineers.'}
                                    </p>
                                </div>

                                <div className="flex items-center justify-between pt-4 border-t border-gray-50 mt-auto">
                                    <span className="text-[10px] font-black text-gray-400 uppercase">
                                        {locPdf.fileSize || t.lh_pdfDocument || 'PDF Document'}
                                    </span>
