...
import { translateDynamicTitle } from '../utils/learningTranslationHelper';
        ...
        <div className="space-y-6">
            <div className="space-y-1">
                <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                    ...
                    {t.lh_nav_certificates || 'My Certificates'}
                </h1>
                <p className="text-xs font-semibold text-gray-500">
                    {t.lh_certificates_subtitle || 'View and share your earned aquaculture completion credentials.'}
                </p>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white p-12 rounded-3xl border border-gray-100 text-center max-w-md mx-auto shadow-sm space-y-4">
                    ...
                    <h3 className="font-extrabold text-gray-900 text-base">{t.lh_no_certs_title || 'No Certificates Earned Yet'}</h3>
                    <p className="text-xs text-gray-500 leading-relaxed">
                        {t.lh_no_certs_desc || 'Complete lessons and pass corresponding quizzes with a minimum of 70% score to generate verified academy certificates.'}
                    </p>
                    <a
                    ...
                    >
                        {t.lh_browse_quizzes || 'Browse Quizzes'}
                    </a>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {certificates.map(cert => {
                        const localizedCourse = translateDynamicTitle(cert.course, language);
                        return (
                            <div key={cert._id} className="p-6 bg-white rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
                                <div>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 className="font-extrabold text-gray-900 text-base">{localizedCourse}</h3>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-gray-50 mt-6">
                                    <span className="flex items-center gap-1 text-[10px] font-bold text-gray-400">
                                        ...
                                        {t.lh_issued || 'Issued'}: {new Date(cert.issuedAt).toLocaleDateString()}
                                    </span>
