...
const EditPostModal = ({ isOpen, onClose, onSuccess, post }) => {
    ...
    useEffect(() => {
        if (post) {
            const cat = (post.category || 'fish').toLowerCase();
            setFormData({
                category: cat,
                fishName: post.fishName || '',
                // For fish: size = fish size; for feed/medicine: size may be packing size (legacy)
                size: post.size || '',
                feedType: post.feedType || '',
                medicineType: post.medicineType || '',
                strength: post.strength || '',
                packingSize: post.packingSize || post.size || '',  // fallback for old records
                ...
                additionalRequirement: post.additionalRequirement || ''
            });
        }
    }, [post]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (cat === 'feed') {
            ...
            data.append('size', formData.packingSize); // backward compat
        }
        if (cat === 'medicine') {
            ...
            <div className="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in fade-in zoom-in duration-200">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-gray-700">{t.requiredQty || 'Required Quantity'}</label>
                            <input
                                type="text"
                                required
                                className="w-full px-4 py-3 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-primary"
                        ...
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-gray-700">{t.budgetCurrency || 'Budget (₹)'}</label>
                            <input
                                type="text"
                                required
                                className="w-full px-4 py-3 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-primary"
                    ...
                    {/* ── Mobile Number ──────────────────────────────────── */}
                    <div className="space-y-1">
                        <label className="text-sm font-bold text-gray-700">{t.phone || 'Mobile Number'}</label>
